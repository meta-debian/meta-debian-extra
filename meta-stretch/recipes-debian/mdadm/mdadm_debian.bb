...
# Build mdadm with source code from stretch-master branch
...
do_compile_prepend() {
	# Replace host include path by sysroot include path
	# to prevent mdadm looking for corosync and libdlm header from host system
	sed -i -e "s@^\(COROSYNC:=.*\)\s*/usr/include@\1${STAGING_INCDIR}@g" \
	       -e "s@^\(DLM:=.*\)\s*/usr/include@\1${STAGING_INCDIR}@g" Makefile
}
...
do_install() {
	...
	# debian/initramfs has been changed in stretch version
	# we need overwrite do_install function to update it.
	install -Dm0755 ${S}/debian/initramfs/hook \
	        ${D}${datadir}/initramfs-tools/hooks/mdadm
	install -Dm0755 ${S}/debian/initramfs/script.local-block \
	        ${D}${datadir}/initramfs-tools/scripts/local-block/mdadm
	install -Dm0755 ${S}/debian/initramfs/script.local-bottom \
	        ${D}${datadir}/initramfs-tools/scripts/local-bottom/mdadm
