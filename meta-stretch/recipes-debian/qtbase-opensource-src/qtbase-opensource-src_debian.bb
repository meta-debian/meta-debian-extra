#
# base recipe: meta-qt5/recipes-qt/qt5/qtbase_git.bb
...
PACKAGECONFIG_GL_arm ?= "${@bb.utils.contains('DISTRO_FEATURES', 'opengl', 'gles2', '', d)}"
...
PACKAGECONFIG_X11 ?= "${@bb.utils.contains('DISTRO_FEATURES', 'x11', \
	'xcb xsync xshape xrender xrandr xfixes xinput2 xcursor glib xkb', '', d)}"
...
PACKAGECONFIG_DISTRO ?= "sql-mysql sql-psql sql-odbc sql-sqlite sql-tds \
                         icu accessibility examples linuxfb cups mtdev"
...
PACKAGECONFIG[sql-mysql] = "-plugin-sql-mysql -mysql_config ${STAGING_BINDIR_CROSS}/mysql_config,-no-sql-mysql,mysql"
PACKAGECONFIG[sql-psql] = "-plugin-sql-psql -I${STAGING_INCDIR}/postgresql,-no-sql-psql,postgresql"
...
cpu_opt_x86 = "-no-use-gold-linker"
...
FILES_libqt5gui = " \
    ...
    ${OE_QMAKE_PATH_LIBS}/libQt5EglDeviceIntegration${SOLIBS} \
    ${OE_QMAKE_PATH_LIBS}/libQt5XcbQpa${SOLIBS} \
    ${OE_QMAKE_PATH_LIBS}/libQt5EglFsKmsSupport${SOLIBS} \
    ...
    ${OE_QMAKE_PATH_PLUGINS}/xcbglintegrations/* \
    ${OE_QMAKE_PATH_PLUGINS}/egldeviceintegrations/* \
