...
do_install_append() {
	...
	install -D -m 0644 ${S}/debian/polkit/60-libvirt.pkla \
		${D}${datadir}/polkit-1/rules.d/60-libvirt.pkla
...
PACKAGES =+ " \
	...
	${PN}-daemon-system \
	${PN}-sanlock"
FILES_libnss-${PN} = "${base_libdir}/libnss_libvirt.so.2"
FILES_${PN}-clients = "\
...
FILES_${PN}-daemon-system = "\
	...
	${localstatedir}/lib/*"
FILES_${PN}-sanlock = " \
	${sysconfdir}/cron.weekly/libvirt-sanlock \
	${libdir}/${PN}/lock-driver/sanlock.so"
FILES_${PN}-dbg += "\
