...
SRC_URI += "file://0001-Add-include-macro.h-to-mtd_probe.h.patch \
            file://0002-Dont-build-manpages-to-reduce-dependency.patch"
...
do_install_append() {
	...
	# fix location of NSS modules follow debian/rules
	for f in ${D}${libdir}/libnss_*; do mv $f ${D}${base_libdir}; done
...
FILES_systemd-coredump = "${sysconfdir}/systemd/coredump.conf \
                          ...
                          ${base_libdir}/systemd/systemd-coredump \
                          ${bindir}/coredumpctl \
                         ...
                         "
FILES_systemd-journal-remote = "${sysconfdir}/systemd/journal-*.conf \
